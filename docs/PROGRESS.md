...
- [x] 멘션 (@) 기능 - Mention 확장 + MentionList 컴포넌트
- [x] 이모지 선택기 - `/emoji` 슬래시 명령어 + EmojiPicker 컴포넌트
- [x] 드래그 앤 드롭 블록 이동 - DragHandle 컴포넌트 (Notion 스타일 개별 아이템 드래그)
- [x] 협업 편집 (Y.js) - CollaborationProvider 컴포넌트
...
#### 위치 정렬 문제 해결
- 핸들이 뷰포트 왼쪽 밖으로 잘리지 않도록 최소 여백 보장 (`MIN_LEFT_MARGIN`)
- 왼쪽 여백 영역에서 마우스 X좌표 자동 조정 (`LEFT_MARGIN_X_OFFSET`)
- 노드 경계 감지 개선 (`parentOffset === 0` 체크, `nodeAt(pos)` 직접 확인)
- TaskItem의 실제 텍스트 요소 위치 기준으로 핸들 정렬
...
- 5가지 드롭 케이스 처리:
  ...
  5. 일반 블록을 다른 위치로 이동
- 드롭 대상 감지 폴백 로직 (빈 paragraph, 블록 사이 드롭 처리)
